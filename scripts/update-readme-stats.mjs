...
function renderProfileHero({
  ...
  const codeAdditions = monthlyCodeStats?.engineering?.additions ?? 0;
  const codeDeletions = monthlyCodeStats?.engineering?.deletions ?? 0;
  const sourceAdditions = monthlyCodeStats?.source?.additions ?? 0;
  const monthLabel = monthlyCodeStats?.monthLabel ?? 'Current Month';
  const monthlyCommitCount = monthlyCodeStats?.nonMergeCommitCount ?? 0;
  ...
  const sourceValue = formatSignedCompact(sourceAdditions);
  const deletedValue = formatSignedCompact(-codeDeletions);
  const monthlyScope = `${monthlyCommitCount} non-merge commits / ${monthlyRepoCount} repos`;
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="560" viewBox="0 0 1200 560" role="img" aria-labelledby="title desc">
  ...
  <g transform="translate(64 252)">
    ...
    <text x="32" y="180" class="micro">LINES ADDED / SOURCE + CONFIG</text>
    <text x="382" y="77" class="small">SOURCE ${escapeXml(sourceValue)}</text>
    ...
    <text x="382" y="176" class="small">${escapeXml(privateScope.toUpperCase())}</text>
  </g>
...
async function main() {
  ...
  const ossSignalBlock = [
    ...
    `      <sub>${monthlyCodeStats?.monthLabel ?? 'Current month'}: ${formatCodeVolume(monthlyCodeStats?.engineering?.additions ?? 0)} source + config lines added.</sub>`,
    '    </td>',
    '    <td width="210">',
